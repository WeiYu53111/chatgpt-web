...
import { UserData, UserHisData } from './interface/userData.interface';
...
export class StatService {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      res.push({
        ...
        create_time: users[i].create_time
          ? moment(users[i].create_time).format('YYYY-MM-DD HH:mm:ss')
          : '',
        last_login_time: users[i].last_login_time
          ? moment(users[i].last_login_time).format('YYYY-MM-DD HH:mm:ss')
          : '',
        usage: query_count,
        limit: 10,
      });
   ...
   * @param userDto
   */
  public async fetchUserHisData(): Promise<UserHisData[]> {
    /*const users = await this.usersService.findAllUser();
    ...
    const stats = await this.statRepository.find({
      order: { squad_date: 'DESC', email: 'ASC' },
    });
    const res = [];
    for (let i = 0; i < stats.length; i++) {
      const record = stats[i];
      res.push({
        squad_date: record.squad_date,
        email: record.email,
        usage: record.query_count,
      });
    }
    return res;
  }
  ...
  public async updateQueryCountByEmail(
    email: string,
    count: number,
  ): Promise<void> {
    const today = Number.parseInt(formatDate(new Date()));
